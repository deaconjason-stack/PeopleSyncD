...
public static class PeopleEndpoints
{
    public static IEndpointRouteBuilder MapPeopleEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/", async (HttpContext http, Guid organizationId, PeopleSyncDDbContext db, TenantAccess tenant, CancellationToken cancellationToken) =>
        {
            ...
            var role = await tenant.GetRoleAsync(http.User, organizationId, cancellationToken);
            if (!TenantAccess.MeetsMinimum(role, MembershipRole.Member)) return Results.Forbid();
            var people = await db.People.AsNoTracking().Where(x => x.OrganizationId == organizationId).OrderBy(x => x.LastName)
                .Select(x => new PersonResponse(x.Id, x.OrganizationId, x.FirstName, x.LastName, x.Email, x.Status.ToString())).ToListAsync(cancellationToken);
        ...
        {
            if (http.User.Identity?.IsAuthenticated != true) return Results.Unauthorized();
            var role = await tenant.GetRoleAsync(http.User, request.OrganizationId, cancellationToken);
            if (!TenantAccess.MeetsMinimum(role, MembershipRole.Manager)) return Results.Forbid();
